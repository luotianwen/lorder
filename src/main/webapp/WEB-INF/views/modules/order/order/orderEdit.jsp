...
<form:form id="inputForm" modelAttribute="order" action="${ctx}/order/order/order/save"  method="post" class="form-horizontal">
	...
	<div class="row control-group">
		...
		<div class="span2 ">
			<sys:treeselect id="province" name="province"    value="${order.province.name}" labelName="province.name" labelValue="${order.province.name}"
							title="区域" url="/sys/area/treeData" cssClass="input-medium required" allowClear="true"  />
		</div>

		<label class="span1 control-label">收货地址-市：</label>
		<div class="span2 ">
			<sys:treeselect id="city" name="city"   value="${order.city.name}" labelName="city.name" labelValue="${order.city.name}"
							title="区域" url="/sys/area/treeData" cssClass="input-medium required" allowClear="true"  />
		</div>

		<label class="span1 control-label">收货地址-区县：</label>
		<div class="span2 ">
			<sys:treeselect id="county" name="county"   value="${order.county.name}" labelName="county.name" labelValue="${order.county.name}"
	...
	<div class="row control-group"><label class="span1 control-label">收货人名称：</label>
		<div class="span2 ">
			<form:input path="consigneeName" htmlEscape="false" maxlength="100" class="input-medium required"  />
		</div>

		<label class="span1 control-label">收货人电话：</label>
		<div class="span2 ">
			<form:input path="consigneePhone" htmlEscape="false" maxlength="100" class="input-medium required"  />
		</div>
