...
<head>
	...
	<script type="text/javascript">
            ...
            $("#btnExport").click(function () {
                top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                    if (v == "ok") {
                        ...
                        $("#searchForm").attr("action", "${ctx}/order/order/order/shipperexport");
                        $("#searchForm").submit();
        ...
        function allReDeliver() {
            ...
            var oldAction = $("#searchForm").attr("action");
            //$("#searchForm").attr("target","_blank");
            $("#searchForm").attr("action", "${ctx}/order/order/order/allReDeliver?type=1&ids=" + delIds);
            $("#searchForm").submit();
            //$("#searchForm").attr("target","_self");
           // $("#searchForm").attr("action", oldAction);
        }
        ...
        function allDeliver() {
            ...
            var oldAction = $("#searchForm").attr("action");
            //$("#searchForm").attr("target","_blank");
            $("#searchForm").attr("action", "${ctx}/order/order/order/allDeliver?type=1&ids=" + delIds);
            $("#searchForm").submit();
            //$("#searchForm").attr("target","_self");
           // $("#searchForm").attr("action", oldAction);
        }
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/order/order/order/shipper">订单管理列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="order" action="${ctx}/order/order/order/shipper" method="post" class="breadcrumb form-search"  >
